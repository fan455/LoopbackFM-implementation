...
if excitation_type == 0:
    ...
    if noise_sr != sr:
        print('The sample rates of direct sound and white noise are not the same.')
        exit()
    use_excitation_envelope = True
    A_e = 0.25
    τ_e = 1000.
else:
    print('No excitation type.')
    exit()
...
if excitation_type == 0:
    ...
    e_sos_arr = butter(2, (f_low, f_high), btype='bandpass', output='sos', fs=sr)
